...
class ExampleMCPServer {
    ...
    async handleFileOperations(args) {
        if (!args || typeof args.operation !== 'string' || typeof args.path !== 'string') {
            throw new McpError(ErrorCode.InvalidParams, "Both 'operation' and 'path' parameters are required");
        }
        ...
        try {
            switch (args.operation) {
                ...
                default:
                    throw new McpError(ErrorCode.InvalidParams, `Unknown file operation: ${args.operation}`);
            }
        }
        catch (error) {
            if (error instanceof McpError) {
            ...
            const errorMessage = error instanceof Error ? error.message : String(error);
            throw new McpError(ErrorCode.InternalError, `File operation failed: ${errorMessage}`);
        }
    }
    setupErrorHandling() {
